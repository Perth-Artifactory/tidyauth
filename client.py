...
t = time.time()
r = requests.get("http://localhost:8080/api/v1/keys/door", params={"token": token})
keys = r.json()
...
t = time.time()
r = requests.get(
    "http://localhost:8080/api/v1/keys/door",
    params={"token": token, "update": "tidyhq"},
)
keys = r.json()
...
if contacts_with_sounds:
    ...
    t = time.time()
    r = requests.get(
        "http://localhost:8080/api/v1/data/sound",
        params={"token": token, "tidyhq_id": str(id)},
    )
    if r.status_code == 200:
        d = r.json()
        print(
            f"TidyHQ contact <{id}> still has a door sound assigned, attempting download"
        )
...
t = time.time()
r = requests.get(
    "http://localhost:8080/api/v1/keys/vending",
    params={"token": token, "update": "tidyhq"},
)
keys = r.json()
...
t = time.time()
r = requests.get(
    "http://localhost:8080/api/v1/data/vending",
    params={"token": token, "update": "tidyhq"},
)
drinks = r.json()
