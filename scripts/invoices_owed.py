...
try:
    r = requests.get(config["urls"]["invoices"],params={"access_token":config["tidytoken"]})
    invoices = r.json()
...
for person in close:
    ...
    if not name:
        name = person
    close[person] = {"name":name,
                     "invoices": close[person]}
for person in outstanding:
    name = util.prettyname(contact_id = person, contacts = contacts)
    if not name:
        name = person
    outstanding[person] = {"name":name,
                           "invoices": outstanding[person]}
...
if dates:
    ...
    for c,f in zip(range(len(dates)),sorted(dates.keys())):
        if f == 0:
            ...
            d[0].append(f'{f}')
        d[1].append(dates[f])
    s.append({"title":"Due membership invoices",
...
if outstanding:
    ...
    s.append({"explainer": "Invoices due 7+ days ago",
              "table": d})
...
if close:
    ...
    for p in sorted(close.items(), key=lambda i: i[1]['invoices'], reverse=True):
        d.append([p[1]["name"],p[1]["invoices"]])
    s.append({"explainer": "Invoices due 74+ days ago",
              "table": d})
