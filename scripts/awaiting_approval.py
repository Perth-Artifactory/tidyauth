...
for membership in memberships:
    if membership["state"] != "expired":
        ...
        if not a:
            problems.append({"name":util.prettyname(contact_id=membership["contact_id"], contacts=contacts),
...
if problems:
    d = [["Name", "Status", "Level"]]
    for problem in problems: d.append([problem["name"],problem["problem"],problem["level"]])
    if output_format == "json":
        pprint(d[1:])
    elif output_format in ["html","mrkdwn","internal"]:
        s = [{"title":"Memberships that need attention",
